...
std::vector<std::vector<FullSite>>  NSPproteinrep::readfullsitesfrompdb(
	...
	for(auto it=sites.begin();it !=sites.end()-1;++it){
		...
		if (bond > 2.0 || it->chainid()!=(it+1)->chainid()){
		...
		for (int i = 0; i < sites[c].size(); ++i) {
			...
			if (vsc.nscatoms>0) {
				for(int m=0;m<vsc.atomnames.size();++m){
					...
					resname,0);
					auto fscrd=fs.getcrds();
					if(fscrd.find(vsc.atomnames[m])==fscrd.end()) continue;
					NSPgeometry::XYZ r=fscrd.at(vsc.atomnames[m]);
...
//label and newlabel only record the start of loop and end of loop
std::vector<std::vector<FullSite>>  NSPproteinrep::readfullsitesfrompdb_xy(const std::string &pdbfilename,
		const std::vector<std::pair<char,std::pair<int,char>>> &label, std::vector<std::pair<int,int>> &newlabel){
	PdbReader reader;
	reader.readpdb(pdbfilename);
	std::string chainids=reader.chainids();
	std::vector<FullSite>  sites;
	//std::set<std::pair<char,std::pair<int,char>>> lb;
	bool islabel{false};
	int ix=0;
	for(int i=0;i<chainids.size();++i){
		char chainid=chainids[i];
		...
		for(int resinumber=0;resinumber<seq.size();++resinumber){
			...
			//if(!keepnonstandard)
			//	if(!SideChainTopo::residuetypedefined(records[0].residuename)) continue;
			//amino acid more than 21s is regarded as GLY
			if(!SideChainTopo::residuetypedefined(records[0].residuename)){
				FullSite fs;
				fs.resname()="GLY";
				fs.resid()=records[0].residueid;
				fs.chainid()=records[0].chainid;
				fs.insertionid()=records[0].insertionid;
				bool an, aca, ac, ao;
				an=aca=ac=ao=false;
				for(auto &r:records) {
					NSPgeometry::XYZ crd(r.x,r.y,r.z);
					if(r.atomname=="N") {an=true;fs.addatomcrd("N",crd);}
					if(r.atomname=="CA") {aca=true;fs.addatomcrd("CA",crd);}
					if(r.atomname=="C") {ac=true;fs.addatomcrd("C",crd);}
					if(r.atomname=="O") {ao=true;fs.addatomcrd("O",crd);}
				}
				if(an&&aca&&ac&&ao) sites.push_back(fs);
				continue;
			}
			FullSite fs;
			fs.resname()=records[0].residuename;
			...
			for(auto &r:records) {
				NSPgeometry::XYZ crd(r.x,r.y,r.z);
				if(r.atomname=="N") an=true;
				if(r.atomname=="CA") aca=true;
				if(r.atomname=="C") ac=true;
				if(r.atomname=="O") ao=true;
				fs.addatomcrd(r.atomname,crd);
			}
			if(an&&aca&&ac&&ao) {
				sites.push_back(fs);
				/*std::pair<char,std::pair<int,char>> ll={chainid,{fs.resid(),fs.insertionid()}};
				if(islabel) {
					lb.insert(ll);
					if(ll==label[ix+1]) {
						islabel=false;
	...
	newlabel.clear();
	std::set<std::pair<char,std::pair<int,char>>> lb;
	for(auto &l:label) lb.insert(l);
	for(auto it=sites.begin();it !=sites.end()-1;++it){
		chains[chainnumber].push_back(*it);
		it->resseq()=resseq++;
		if(lb.find({it->chainid(),{it->resid(),it->insertionid()}})!=lb.end()) newlabel.push_back({chainnumber,resseq});
		NSPgeometry::XYZ c0=it->getcrd("C");
		NSPgeometry::XYZ n1=(it+1)->getcrd("N");
		double bond = NSPgeometry::distance(c0, n1);
		if (bond > 2.0 || it->chainid()!=(it+1)->chainid()){
		...
		if(it==sites.end()-2) {
			chains[chainnumber].push_back(*(it+1));
			(it+1)->resseq()=resseq++;
			if(lb.find({(it+1)->chainid(),{(it+1)->resid(),(it+1)->insertionid()}})!=lb.end()) newlabel.push_back({chainnumber,resseq});
		}
	}
	return chains;
}
...
void FullSite::changeaa(std::string newaa) {
	restype_ = newaa;
