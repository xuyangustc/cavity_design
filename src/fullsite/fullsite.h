...
//	std::map<std::string,std::vector<double>> internalcrds()const;
	...
	void changecrd(std::map<std::string,NSPgeometry::XYZ> &cs){crds_=cs;}
	void covert2backbone() {
		restype_ = "GLY";
		static std::set<std::string> bba;
		if(bba.empty()) {
			bba.insert("N");
			bba.insert("CA");
			bba.insert("C");
		...
		std::map<std::string,NSPgeometry::XYZ> temp;
		for(auto &c:crds_) if(bba.find(c.first)!=bba.end()) temp.insert(c);
		crds_ = temp;
	}
	std::map<std::string,NSPgeometry::XYZ> getcrds() {return crds_;}
	const std::map<std::string,NSPgeometry::XYZ> getcrds() const {return crds_;}
	char &insertionid() {return insertionid_;}
	const char &insertionid() const {return insertionid_;}
	void changeaa(std::string newaa);
private:
...
std::vector<std::vector<FullSite>>  readfullsitesfrompdb_xy(const std::string &pdbfilename,
