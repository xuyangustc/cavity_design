...
struct PdbRecord {
	...
	std::string elem() const {
		std::string s;
		if(elementname[0]!=' ') s.push_back(elementname[0]);
		if(elementname[1]!=' ') s.push_back(elementname[1]);
		return s;
	}
};
...
PdbRecord make_pdbrecord(const typename ATOMKEY::Key & key, const XYZ & xyz,int atomid=0, int residueidshift=0,
	...
	PdbRecord record;
	const std::vector<char> chainids{'A','B','C','D','E','F','G','H','I','J','K','L','M','N',
	...
	if(chainid <26) record.chainid=chainids[chainid];
