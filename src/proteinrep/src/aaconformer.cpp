...
AAConformer NSPproteinrep::make_aaconformer(const std::vector<PdbRecord> & residuerecords,
	...
	for(auto & a:residuerecords) {
		if(a.conformerid == states_v[0]) aa.globalcrd.insert(std::make_pair(a.atomname,XYZ(a.x,a.y,a.z)));
...
std::vector<PdbRecord> AAConformer::make_pdbrecords(char chainid,int resid,char insertionid,int atomid0){
	...
	for(auto & atm:atomnames) {
		...
		rec.x=globalcrd[atm].x_;
		rec.y=globalcrd[atm].y_;
		rec.z=globalcrd[atm].z_;
	}
	return res;
}
