...
struct  BondTerm
{
	...
	std::vector<int> lab() const {return {i,j};}
};
...
struct AngleTerm{
	...
	double energy(const std::vector<NSPgeometry::XYZ> &crd,
			std::vector<NSPgeometry::XYZ> *forces) const;
	std::vector<int> lab() const {return {i,j,k};}
};
...
struct ImpDihTerm {
	...
	std::vector<int> lab() const {return {i,j,k,l};}
};
...
struct PhiPsi {
	...
	enum amino_acid_20 {AA18,GLY,PRO};
...
//			std::vector<NSPgeometry::XYZ> *forces) const;
	void changeaa(char c) {
		if(c=='P') aa20=PRO;
		else if(c=='G') aa20=GLY;
	}
...
struct EnergyTerm {
	...
	std::vector<std::pair<MultiAtom,double>> ebond, eang, eimpdih, esteric;
	std::vector<std::pair<SingleResidue,double>> ephipsi, elocal, escconf;
	std::vector<std::pair<MultiResidue,double>> esitepair, escpacking, elocalbbhb;
	void clear() {
		ebond.clear();
		eang.clear();
		eimpdih.clear();
...
//			std::vector<NSPgeometry::XYZ> *xyzf) const;
	...
	double lsenergy_subentry(const std::vector<PhiPsiCodes> & phipsicodes,
			const std::vector<double> &siteweight, std::vector<NSPgeometry::XYZ> *xyzf,
...
//			std::vector<std::vector<NSPgeometry::XYZ>> &xyzf_thread) const;
	...
	template <typename CVLTERM>
	double covalentenergy_sbuentry(const std::vector<CVLTERM> &cvlterms, const std::vector<NSPgeometry::XYZ> &xyz,
			std::vector<NSPgeometry::XYZ> *xyzf, std::vector<std::pair<std::vector<int>,double>> &esub,
			 const std::vector<bool> *forceoff=nullptr) const {
		double ene=0.0;
		for(auto &t:cvlterms){
			double etemp=t.energy(xyz,xyzf);
			esub.push_back({t.lab(),etemp});
			if(t.off(forceoff))continue;
			ene += etemp;
		}
		return ene;
...
#pragma omp parallel for schedule(dynamic,1)
	 ...
	 double stericenergy (const NeighborList &nbl,const std::vector<NSPgeometry::XYZ> &xyz,
			 std::vector<NSPgeometry::XYZ> *xyzf) const;
	 double stericenergy_subentry(const NeighborList &nbl, const std::vector<NSPgeometry::XYZ> &xyz,
			 std::vector<NSPgeometry::XYZ> *xyzf, std::vector<std::pair<std::vector<int>,double>> &esub) const;
#ifdef _OPENMP
...
/*	double sitepairenergy(const std::vector<double> & crd,
	...
	double sitepairenergy_subentry(const std::vector<double> & crd,
			const std::vector<std::vector<PhiPsiCodes>> & phipsicodes,
			const std::vector<std::vector<SSCode>> &sscodes,
			std::vector<NSPgeometry::XYZ> *xyzf, double *eattr, const ActiveSelections &acts,
...
/*	double sideconfene(const std::vector<double> &crd,
	...
	double sideconfene_subentry(const std::vector<double> &crd, const std::vector<std::vector<PhiPsiCodes>> &phipsicodes,
...
/*	double scpackingene(const std::vector<NSPgeometry::XYZ> &xyz,
	...
	double scpackingene(const std::vector<NSPgeometry::XYZ> &xyz,
				std::vector<NSPgeometry::XYZ> *xyzf,const ActiveSelections &acts) const;
	double scpackingene_subentry(const std::vector<NSPgeometry::XYZ> &xyz, std::vector<NSPgeometry::XYZ> *xyzf,
...
/*	std::vector<double> scpackingene(const std::vector<NSPgeometry::XYZ> &xyz,
	...
	std::vector<double> scpackingene(const std::vector<NSPgeometry::XYZ> &xyz,
			std::vector<std::vector<NSPgeometry::XYZ>> &xyzf_thread,
			const ActiveSelections &acts) const;
	std::vector<double> scpackingeneNN(const std::vector<NSPgeometry::XYZ> &xyz,
			std::vector<std::vector<NSPgeometry::XYZ>> &xyzf_thread,
	...
	double localbbhbenergy_subentry(const std::vector<NSPgeometry::XYZ> &xyz, std::vector<NSPgeometry::XYZ> *xyzf,
			std::vector<std::pair<std::vector<std::vector<int>>,double>> &esub) const;
	...
	double getmcrmsd(const std::vector<double> &conf, const std::vector<double> &crd,
			const std::vector<bool> &forceoff, double nm2a=1.0) const;
	double getallrmsd(const std::vector<double> &conf, const std::vector<double> &crd,
			const std::vector<bool> &forceoff, double nm2a=1.0) const;
	double forces_rmsd(const std::vector<double> &crd, std::vector<double> &ffs, const std::vector<bool> &forceoff,
			const std::vector<std::vector<double>> &confs, double findex, bool mconly) const;
	double forces_rmsd(const std::vector<double> &crd, std::vector<double> &ffs, const std::vector<bool> &forceoff,
			const std::vector<std::vector<double>> &confs, double findex, double rmsdcutoff, bool mconly) const;
	double forces_rmsd_ene(const std::vector<double> &crd, std::vector<double> &ffs,
			const std::vector<bool> &forceoff, const std::vector<std::vector<double>> &confs,
			double findex, bool mconly, double rmsdcutoff, int &cfminrmsd) const;
	//std::vector<double> forces(const std::vector<double> &crd, const NeighborList &nbl,
	//		std::vector<double> *potenergies, ActiveSelections &acts, const std::vector<bool> &forceoff) const;
	std::vector<double> forces_beta_sheet(const std::vector<double> &crd, const NeighborList &nbl,
	...
	void changephipsiaa(int i, char c) {phipsis_[i].changeaa(c);}
	std::vector<double> forces_strand_unbend(const std::vector<double> &crd,
	...
	std::vector<double> forces_strand_hb(const std::vector<double> &crd,
			const NeighborList &nbl, std::vector<double> *potenergies, ActiveSelections &acts,
			const std::vector<std::pair<int,int>> &hbpairs, double discutoff, double f_index) const;
	std::vector<double> forces_subentry(const std::vector<double> &crd, const NeighborList &nbl,
	...
	const std::vector<NSPallatom::ChemGroup> &cgps() const {return chemgroups_;}
	void addchemgroup(NSPallatom::ChemGroup &cg) {
		chemgroups_.push_back(cg);
	}
	double stericwght() {return stericwght_;}
	struct Fixedpoint_Attraction_Repulsion {
		std::vector<int> atoms_seq;
		std::vector<XYZ> fixed_points;
		double f_rep, f_att; //force should be N average of total force, N is equal to size of fixed_points
		std::vector<double> dis; //size==6
		double getf(double mind, double maxd, double f, double dis1) const {
			if(dis1<mind) return 0.0;
			if(dis1>maxd) return f;
			double a = maxd-mind;
			a *= a;
			a = 2.0 * f / a;
			double mm = (mind+maxd)/2.0;
			if(dis1<mm) {
				double delta = dis1-mind;
				return delta*delta*a;
			} else {
				double delta = maxd - dis1;
				return f - a * delta * delta;
			}
		}
		bool getf(double d, double &f) const {//false->repulsion, true->attraction
			if(d<dis[0]) {
				f= f_rep;
				return false;
			} else if(d<dis[1]) {
				f= f_rep-getf(dis[0],dis[1],f_rep,d);
				return false;
			} else if(d<dis[2]) {
				f=0.0;
				return false;
			} else if(d<dis[3]) {
				f= getf(dis[2],dis[3],f_att,d);
				return true;
			} else if(d<dis[4]) {
				f= f_att;
				return true;
			} else if(d<dis[5]) {
				f= f_att-getf(dis[4],dis[5],f_att,d);
				return true;
			} else {
				f= 0.0;
				return true;
	...
	struct Site_Restrict_RMSD {
		//one fixed site attract one SD site
		//if all SD sites are far away from one site, then this site will attract all SD sites
		std::vector<std::vector<XYZ>> fixed; //each site contain 4 crds: N, CA, C, O
		double dis_near, dis_far; //0<1
		double f_near, f_far; //d<d0 -> f,  d>d1 -> 0, make sure f_near > f_far
		std::vector<std::vector<int>> nbb; //atom serial number in a site as a vector
		void getf(std::vector<XYZ>&xyzf, const std::vector<XYZ>&crds) const;
		XYZ attractforce(const XYZ &qc, const XYZ &fx) const {
			XYZ v =fx-qc;
			double d=sqrt(v.squarednorm());
			if(d<0.01) return XYZ(0.0,0.0,0.0);
			if(d<dis_near) return v/d*f_near;
			if(d>dis_far) return v/d*f_far;
			double half=dis_near+dis_far;
			half /= 2.0;
			double a=f_near-f_far;
			a*=2.0;
			a/=(dis_near-dis_far);
			a/=(dis_near-dis_far);
			if(d<half) {
...
#ifdef _OPENMP
	//set attract forces between 2 atoms
	//sub vector of attrs is serial number of 2 atoms
	std::vector<double> forces_attract(const std::vector<double> &crd, const NeighborList &nbl, const GroupNeighbor &gn,
			std::vector<double> *potenergies, ActiveSelections &acts, const std::vector<std::pair<std::vector<int>,double>>&attrs) const;
	std::vector<double> forces_fixedpoint_restrict(const std::vector<double> &crd,
			const NeighborList &nbl, const GroupNeighbor &gn, std::vector<double> *potenergies,
			ActiveSelections &acts, const std::vector<std::pair<std::pair<std::vector<int>,
			XYZ>,std::vector<double>>>&pointinfo) const;
	std::vector<double> forces_fixedpoint_attraction_repulsion(
			const std::vector<double> &crd, const NeighborList &nbl, const GroupNeighbor &gn,
			std::vector<double> *potenergies, ActiveSelections &acts,
			const std::vector<Fixedpoint_Attraction_Repulsion> &pointinfo,
...
private:
	...
	std::vector<NSPallatom::ChemGroup> chemgroups_;
...
struct GroupNeighbor {
	...
	GroupNeighbor(const std::vector<double> &crd, const ForceField &ff);
};
...
ForceField make_forcefield_allatom(const std::vector<std::vector<std::string>> &scnames,
		const std::string &controlname, std::vector<std::vector<int>> &cissites,
		std::vector<std::vector<std::map<std::string,int>>> &serial);
std::vector<std::vector<int>> getcissites(const std::vector<std::vector<NSPproteinrep::BackBoneSite>> &bsss,
		const std::vector<std::set<int>> &notcis);
ForceField make_forcefield_allatom(const std::vector<std::vector<NSPproteinrep::BackBoneSite>> &bsss,
		const std::string &controlname, std::vector<std::vector<std::map<std::string,int>>> &serial,
		std::vector<std::set<int>> notcis=std::vector<std::set<int>>());
ForceField make_forcefield_allatom(const std::vector<std::vector<NSPproteinrep::FullSite>>&fsss,
		const std::string &controlname, std::vector<std::vector<std::map<std::string,int>>> &serial,
		std::vector<std::set<int>> notcis=std::vector<std::set<int>>());
ForceField make_forcefield_allatom(const std::vector<std::vector<NSPallatom::Residue>>&ress,
		const std::string &controlname, std::vector<std::vector<std::map<std::string,int>>> &serial,
		std::vector<std::set<int>> notcis=std::vector<std::set<int>>());
