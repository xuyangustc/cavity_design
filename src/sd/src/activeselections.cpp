...
void ActiveSelections::init(const ForceField *ff, const std::map<int, std::vector<int>> & allfixedcp,
		const std::map<int, std::vector<int>> &mcfixedcp) {
	ff_ = ff;
	const std::vector<std::vector<BSInChain>> &bsinchains = ff_->bsinchains();
	int nchains = bsinchains.size();
	//std::map<int, std::vector<int>> allfixedcp = chainpositionselections(allfixed);
	//std::map<int, std::vector<int>> mcfixedcp = chainpositionselections(mcfixed);
	posistatus_.assign(nchains, std::vector<PosiStatus>());
	//initially, all positions in all chains active
	for (int ic = 0; ic < nchains; ++ic) {
		int cl = bsinchains[ic].size();
