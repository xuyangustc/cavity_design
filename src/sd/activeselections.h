...
public:
	...
	void init(const ForceField *ff, const std::map<int, std::vector<int>> & allfixedcp,
			const std::map<int, std::vector<int>> &mcfixedcp);
	ActiveSelections(const ForceField *ff,const std::map<int, std::vector<int>> & allfixedcp,
			const std::map<int, std::vector<int>> & mcfixedcp){
		init(ff,allfixedcp,mcfixedcp);
	}
	const ForceField * ff() const {return ff_;}
private:
