...
//	static std::string attrib_name;
	...
	NSPgeometry::XYZ getnextN() const {
		IdealGeometries & igdat = IdealGeometries::getGlobalInstance();
		return NSPgeometry::InternaltoXYZ(ccrd(), cacrd(), ocrd(), igdat.idealLength("C", "N"),
					igdat.idealAngle("CA", "C", "N"), 3.14159265);
	}
	NSPgeometry::XYZ getprevC() const {
		IdealGeometries & igdat = IdealGeometries::getGlobalInstance();
		double degree = 3.14159265 / 180.0;
		return NSPgeometry::InternaltoXYZ(ncrd(), cacrd(), ccrd(), igdat.idealLength("C", "N"),
					igdat.idealAngle("C", "N", "CA"), phi() * degree);
	}
	NSPgeometry::XYZ getprevO(double omiga=180.0) const {
		IdealGeometries & igdat = IdealGeometries::getGlobalInstance();
		double degree = 3.14159265 / 180.0;
		return NSPgeometry::InternaltoXYZ(getprevC(), ncrd(), cacrd(), igdat.idealLength("C", "O"),
								igdat.idealAngle("O", "C", "N"), (180.0-omiga) * degree);
	}
	BackBoneSite PrevSite(double psip, double phi0, bool iscis0) const {
		IdealGeometries & igdat = IdealGeometries::getGlobalInstance();
		double degree = 3.14159265 / 180.0;
		NSPgeometry::XYZ pc = NSPgeometry::InternaltoXYZ(ncrd(), cacrd(), ccrd(), igdat.idealLength("C", "N"),
				igdat.idealAngle("C", "N", "CA"), phi0 * degree);
		NSPgeometry::XYZ pca;
		if(iscis0) {
			pca = NSPgeometry::InternaltoXYZ(pc, ncrd(), cacrd(), igdat.idealLength("CA", "C"),
							igdat.idealAngle("CA", "C", "N"), -3.14159265);
		} else {
			pca = NSPgeometry::InternaltoXYZ(pc, ncrd(), cacrd(), igdat.idealLength("CA", "C"),
							igdat.idealAngle("CA", "C", "N"), 3.14159265);
		}
		NSPgeometry::XYZ pn = NSPgeometry::InternaltoXYZ(pca, pc, ncrd(), igdat.idealLength("CA", "N"),
				igdat.idealAngle("N", "CA", "C"), psip * degree);
		NSPgeometry::XYZ po = NSPgeometry::InternaltoXYZ(pc, ncrd(), cacrd(), igdat.idealLength("C", "O"),
				igdat.idealAngle("O", "C", "N"), 3.14159265);
		BackBoneSite bsp;
		bsp.data_[NCRD] = pn.x_;
		bsp.data_[NCRD+1] = pn.y_;
		bsp.data_[NCRD+2] = pn.z_;
		bsp.data_[OCRD] = po.x_;
		bsp.data_[OCRD+1] = po.y_;
		bsp.data_[OCRD+2] = po.z_;
		bsp.data_[CACRD] = pca.x_;
		bsp.data_[CACRD+1] = pca.y_;
		bsp.data_[CACRD+2] = pca.z_;
		bsp.data_[CCRD] = pc.x_;
		bsp.data_[CCRD+1] = pc.y_;
		bsp.data_[CCRD+2] = pc.z_;
		return bsp;
	}
	BackBoneSite NextSite(bool iscisn, double phin, double psin) const {
		IdealGeometries & igdat = IdealGeometries::getGlobalInstance();
		double degree = 3.14159265 / 180.0;
		NSPgeometry::XYZ nn = getnextN();
		NSPgeometry::XYZ nca;
		if(iscisn) {
			nca = NSPgeometry::InternaltoXYZ(nn, ccrd(), cacrd(), igdat.idealLength("CA", "N"),
										igdat.idealAngle("CA", "C", "N"), -3.14159265);
		} else {
			nca = NSPgeometry::InternaltoXYZ(nn, ccrd(), cacrd(), igdat.idealLength("CA", "N"),
										igdat.idealAngle("CA", "C", "N"), 3.14159265);
		}
		NSPgeometry::XYZ nc = NSPgeometry::InternaltoXYZ(nca, nn, ccrd(), igdat.idealLength("CA", "C"),
				igdat.idealAngle("N", "CA", "C"), phin * degree);
		NSPgeometry::XYZ nnn = NSPgeometry::InternaltoXYZ(nc, nca, nn, igdat.idealLength("N", "C"),
				igdat.idealAngle("CA", "C", "N"), psin * degree);
		NSPgeometry::XYZ no = NSPgeometry::InternaltoXYZ(nc, nnn, nca, igdat.idealLength("O", "C"),
				igdat.idealAngle("O", "C", "N"), 3.14159265);
		BackBoneSite bsn;
		bsn.data_[NCRD] = nn.x_;
		bsn.data_[NCRD+1] = nn.y_;
		bsn.data_[NCRD+2] = nn.z_;
		bsn.data_[OCRD] = no.x_;
		bsn.data_[OCRD+1] = no.y_;
		bsn.data_[OCRD+2] = no.z_;
		bsn.data_[CACRD] = nca.x_;
		bsn.data_[CACRD+1] = nca.y_;
		bsn.data_[CACRD+2] = nca.z_;
		bsn.data_[CCRD] = nc.x_;
		bsn.data_[CCRD+1] = nc.y_;
		bsn.data_[CCRD+2] = nc.z_;
		return bsn;
	}

	void resetpsi() {
		NSPgeometry::XYZ nn=getnextN();
		double t = NSPgeometry::torsion(getcrd(NCRD), getcrd(CACRD), getcrd(CCRD), nn);
	...
	static std::vector<NSPgeometry::XYZ> PeptideUnit() {
		IdealGeometries & igdat = IdealGeometries::getGlobalInstance();
		NSPgeometry::XYZ ca2(0.0,0.0,0.0);
		NSPgeometry::XYZ n = NSPgeometry::InternaltoXYZ(ca2,igdat.idealLength("CA", "N"));
		NSPgeometry::XYZ c = NSPgeometry::InternaltoXYZ(n,ca2,
				igdat.idealLength("C","N"),igdat.idealAngle("C", "N", "CA"));
		NSPgeometry::XYZ o = NSPgeometry::InternaltoXYZ(c,n,ca2,igdat.idealLength("O","C"),
				igdat.idealAngle("O", "C", "N"),0.0);
		NSPgeometry::XYZ ca1 = NSPgeometry::InternaltoXYZ(c,n,ca2,igdat.idealLength("CA","C"),
				igdat.idealAngle("CA", "C", "N"),igdat.idealTorsion("CA", "C", "N","CA"));
		return {ca1,c,o,n,ca2};
	}
	/*static std::vector<NSPgeometry::XYZ> PeptideUnit_cis() {
		IdealGeometries & igdat = IdealGeometries::getGlobalInstance();
		NSPgeometry::XYZ ca2(0.0,0.0,0.0);
		NSPgeometry::XYZ n = NSPgeometry::InternaltoXYZ(ca2,igdat.idealLength("CA", "N"));
		NSPgeometry::XYZ c = NSPgeometry::InternaltoXYZ(n,ca2,
				igdat.idealLength("C","N"),igdat.idealAngle("C", "N", "CA"));
		NSPgeometry::XYZ o = NSPgeometry::InternaltoXYZ(c,n,ca2,igdat.idealLength("O","C"),
				igdat.idealAngle("O", "C", "N"),igdat.idealRelativeTorsion("CA", "C", "O","N"));
		NSPgeometry::XYZ ca1 = NSPgeometry::InternaltoXYZ(c,n,ca2,igdat.idealLength("CA","C"),
				igdat.idealAngle("CA", "C", "N"),igdat.idealTorsion("CA", "C", "N","CA"));
		return {ca1,c,o,n,ca2};
	}*/
...
void genprevbackbonesite_phiislast(BackBoneSite *nsite,double omiga,double psi, double phi,
		BackBoneSite *newsite);
