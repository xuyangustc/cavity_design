...
CloseALoop::CloseALoop(const std::vector<BackBoneSite> & hostchain, int loopstart,
		int looplength, std::vector<BackBoneSite> &refloop) : hostchain_(&hostchain),
		loopstart_ ( loopstart ), looplength_(looplength), refloop_(refloop) {
	assert(loopstart_ > 0);
	assert(looplength_ >= 3);
	assert(loopstart_ + looplength_ < hostchain_->size());
	for (int i = 0; i < looplength_; ++i) {
		refloop_[i].resname = (*hostchain_)[loopstart_ + i].resname;
	}
	refloop_.back().data_[BackBoneSite::OMIGA] = (*hostchain_)[loopstart_
			+ looplength_].omiga();
	fixcrds_.clear();
